...
pre_install_docker_compose(){
    echo "面板选择 SSpanel 0， SSRpanel 1"
   ...
   # Set Setting if the node go downwith panel
    ...
    echo
    echo "---------------------------"
    echo "sspanel数据库端口 = ${v2ray_mysqlport}"
    echo "---------------------------"
    echo
    # Set Setting if the node go downwith panel
    echo "数据库名字"
    ...
    echo "（可以不用管）use sspanel speedtest"
    ...
    echo "（可以不用管）Setting V2ray Grpc API Listen port"
    ...
    # Set Setting if the node go downwith panel
    echo "（可以不用管）Setting if the node go downwith panel"
...
pre_install_caddy(){
    ...
    echo "（下面可以不管了，一路回车吧）caddy v2ray path"
